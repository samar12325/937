...
function guardDirector(){
  ...
  const token = localStorage.getItem('token');
  console.log('[DIRECTOR GUARD] Checking user access...', { userRaw, token });
  
  if (!token || !userRaw) {
    console.log('[DIRECTOR GUARD] No token or user data, redirecting to login');
  ...
  console.log('[DIRECTOR GUARD] User data:', { user, roleId });
  
  // السماح للمدير والموظف بالوصول
  if (roleId !== 2 && roleId !== 3 && user.Username?.toLowerCase() !== 'employee' && user.Username?.toLowerCase() !== 'director'){
    console.log('[DIRECTOR GUARD] Access denied, redirecting to home');
    window.location.href = '../login/home.html';
    return false;
  }
  
  console.log('[DIRECTOR GUARD] Access granted for role:', roleId);
